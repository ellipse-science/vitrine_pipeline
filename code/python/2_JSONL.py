...
2) Lit et équilibre les données du CSV (50 % EN, 50 % FR) et les divise en
   parties communes (20 %) et uniques (80 %).
...
def main():
    """
    Fonction principale pour orchestrer la création des fichiers JSONL et du fichier de configuration Doccano.
    ...
    Étapes :
        ...
        3) Diviser les données en 20 % communes et 80 % uniques.
        4) Créer un fichier JSON de configuration Doccano avec tous les labels détectés.
        5) Générer des fichiers JSONL (un par annotateur) avec la répartition correcte.
    ...
    #    (Assumes columns: [id, extraction_date, body, media_id, lang,
    #                       body_id_for_llm, body_annotated])
    # --------------------------------------------------------------------------
    data_rows = []
    ...
    # --------------------------------------------------------------------------
    # 4. Division en 20 % de données communes et 80 % de données uniques
    # --------------------------------------------------------------------------
    ...
    actual_common_count = len(final_common_data)

    # --------------------------------------------------------------------------
    # 5. Répartir les données uniques entre les annotateurs en garantissant 50-50 EN-FR
    # --------------------------------------------------------------------------
    ...
    # Calculer le nombre de lignes uniques par annotateur
    ...
    # --------------------------------------------------------------------------
    # 6. Rassembler tous les labels à partir de "body_annotated"
    # --------------------------------------------------------------------------
    all_labels = set()
    for row in data_rows:
        try:
            annotated_json = json.loads(row.get("body_annotated", "{}"))
            themes = annotated_json.get("themes", [])
            for theme in themes:
                all_labels.add(theme)
    ...
    # --------------------------------------------------------------------------
    # 8. Générer les fichiers JSONL pour chaque annotateur (données communes + uniques)
    # --------------------------------------------------------------------------
    ...
    for i in range(num_annotators):
        ...
        combined_annotator_data = final_common_data + annotators_unique_data[i]
        ...
        with open(file_path, mode="w", encoding="utf-8") as out_f:
            for row in combined_annotator_data:
                text = row.get("body", "")
                try:
                    ann_json = json.loads(row.get("body_annotated", "{}"))
                    themes = ann_json.get("themes", [])
                except json.JSONDecodeError:
                    themes = []

                # Doccano format: {"text": "...", "labels": [...]}
                doccano_line = {"text": text, "label": themes}
    ...
    print(f"Lignes communes (partagées par tous) : {actual_common_count}")
    print(f"Lignes uniques totales : {unique_count}")
    print(f"Lignes uniques par annotateur (approx.) : {unique_count_per_annot}")
