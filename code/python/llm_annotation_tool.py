...
3) Chargement, vérification et analyse détaillée d’un prompt annoté.
...
import re
import json
import subprocess  # (NOUVEAU) pour lister les modèles Ollama
...
def add_annotation_column_df(df, annotation_column):
    """
    Ajoute la colonne d'annotation au DataFrame (CSV/Excel/Parquet/RData/rds) si elle n'existe pas déjà.
    (Renommage de l'ancienne add_annotation_column_csv pour usage générique.)
    """
...
# (NOUVEAU) Fonctions pour lire et écrire différents formats
...
def write_data_file(df, file_path, file_format):
    ...
    Opération (ici) faite après chaque annotation pour ne rien perdre.
    """
    file_format = file_format.lower()
    ...
    if file_format == 'csv':
        ...
        # pyreadr.write_rdata prend un dict {nom_objet: dataframe}
...
def list_ollama_models():
    ...
    Extrait uniquement le vrai nom du modèle (ex. 'nemotron:latest', 'deepseek-r1:70b')
    pour pouvoir l'utiliser dans un appel à 'ollama generate'.
    """
    try:
        ...
        for line in lines:
            ...
            if "NAME" in line and "MODIFIED" in line:
                # C'est probablement l'entête (NAME, ID, SIZE, MODIFIED)
                continue
            if not line.strip():
                # Ligne vide
                continue

            # Exemple de ligne possible : "2. deepseek-r1:70b    0c1615a8ca32    42 GB    10 days ago"
            # On peut la découper selon les espaces
            parts = line.split()

            # Si la première partie contient "1.", "2.", etc., on l'ignore
            # On va prendre la 1ère partie *après* ce nombre + '.' ou l’élément d’indice 0
            # si la liste ne contient pas de numérotation
            # Un moyen simple : vérifier si parts[0].endswith('.'), alors le modèle est parts[1], sinon parts[0].
    ...
    if data_source in ['csv', 'excel', 'parquet', 'rdata', 'rds']:
        ...
        if calculate_ic:
            ...
            if choice_ic == 'variable':
                ...
                # On pourrait faire un nunique pour calculer ?
                # Ex: total_comments = df[unique_var].nunique()
                # Pour simplifier, on reprend juste len(df):
                pass
        ...
        # (NOUVEAU) On demande le chemin de sortie, afin de sauvegarder à chaque ligne
            ...
            try:
                ...
                with tqdm(total=total_to_annotate, desc='Analyse des données', unit='annotation') as pbar:
                    ...
                    for future in concurrent.futures.as_completed(futures):
                        ...
                        # Mise à jour et sauvegarde immédiate
                        if output is not None:
                            df = update_annotation_df(df, identifier, identifier_column, annotation_column, output)

                            # (NOUVEAU) Sauvegarde dans le fichier cible après chaque annotation
                            try:
        ...
        if calculate_ic:
            ...
            if choice_ic == 'variable':
                ...
                while unique_var not in columns:
                    print("Colonne inexistante. Réessayez.")
                    unique_var = input("Variable unique : ").strip()
                # Même remarque : on pourrait faire un distinct, etc.
            ...
            try:
                ...
                with tqdm(total=total_to_annotate, desc='Analyse des données', unit='annotation') as pbar:
                    ...
                    for future in concurrent.futures.as_completed(futures):
                        identifier, output = future.result()
                        if output is not None:
                            # Mise à jour en base de données directement (ligne par ligne)
