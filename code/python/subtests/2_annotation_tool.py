...
8) (NOUVEAU) Itération sur tous les prompts .txt d'un dossier, pour créer
...
import re
import json
import subprocess  # pour lister les modèles Ollama
...
def load_prompt(prompt_path):
    ...
    (Pour cette nouvelle version, on ne découpe plus en 'expected_keys',
     car nous allons juste extraire le thème unique via parse_theme_name.)
    """
...
def parse_theme_name(prompt_text):
    ...
    """
    # Cherche un bloc :  "themes": ["quelque_chose"]
    # On se base sur la forme  "themes": ["..."]
    # Éventuellement, on tolère les guillemets doubles et simples.
    # Simplifions : on cherche strictement "themes": ["quelque_chose"].
...
def main():
    ...
    if data_source in ['csv', 'excel', 'parquet', 'rdata', 'rds']:
        ...
        if calculate_ic:
            ...
            # On ne réutilise pas spécialement la 'variable' ici, 
            # mais on respecte la structure existante.
            if choice_ic == 'variable':
                ...
                # Possibilité d'utiliser df[unique_var].nunique() si besoin.
        ...
        for pf in prompt_files:
            ...
            if not theme_name:
                # Si on ne trouve pas de thème, on peut passer ou 
                # simplement donner un nom par défaut
            ...
            # Création (ou vérification) de la colonne correspondante
                    ...
                    with tqdm(total=total_to_annotate, desc=f'Analyse ({pf})', unit='annotation') as pbar:
                        ...
                        for future in concurrent.futures.as_completed(futures):
                            ...
                            if output is not None:
                                df = update_annotation_df(df, identifier, identifier_column, theme_name, output)
    ...
    ###########################################################################
        ...
        # 12) Pour chaque prompt, on crée la colonne, on annote
        for pf in prompt_files:
            full_prompt_path = os.path.join(prompt_dir, pf)
            base_prompt = load_prompt(full_prompt_path)
            theme_name = parse_theme_name(base_prompt)
            ...
            # On crée la colonne dans la table
        ...
        with engine.connect() as connection:
            annotated_query = text(f"""
                SELECT {identifier_column}, {text_column},
                       {', '.join([col for col in columns if col != text_column and col != identifier_column])}
                FROM {table_name}
            """)
            # Remarque : On sélectionne a minima la colonne texte, l'identifiant 
            # et toutes les autres colonnes. 
            # Les nouvelles colonnes JSONB créées (theme_name) devraient y figurer.
